	...
	if [[ $os == "ubuntu" ]]; then
		if [[ $(docker ps -a --format '{{.Names}}' | grep -w core-v9) == "core-v9" ]]; then
			dateis=$(date +%d%m%Y)
                ...
                for a in ${!Months[@]}
                do
                    ./hadoop fs -du -h /DNIF/events/Scope=default/Stream=TRAFFIC/Year=2022/Month=${Months[$a]} >> /var/tmp/dfstraffic2022size__$dateis"T"$timeis.out
                done
            echo -e "\ndfslist report generated successfully!!!\n"
			echo -e "path: /var/tmp/dfstraffic2022size__$dateis"T"$timeis.out\n"
		else
