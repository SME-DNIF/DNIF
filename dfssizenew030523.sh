...
#Months=("202206" "202207" "202208" "202209" "202210" "202211" "202212")
	...
	if [[ $os == "ubuntu" ]]; then
		if [[ $(docker ps -a --format '{{.Names}}' | grep -w core-v9) == "core-v9" ]]; then
			dateis=$(date +%d%m%Y)
                ...
                #for a in ${!Months[@]}
                #do
                ./hadoop fs -du -h /DNIF/events/Scope=default/Stream=FIREWALL/Year=2022 >> /var/tmp/dfslistsizeyear_$dateis"T"$timeis.out
                #done
            echo -e "\ndfslist report generated successfully!!!\n"
			echo -e "path: /var/tmp/dfslistsizeyear_$dateis"T"$timeis.out\n"
		else
